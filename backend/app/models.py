...
class ToolApp(Base):
    ...
    invites: Mapped[list["AppInvite"]] = relationship(
        back_populates="app",
        cascade="all, delete-orphan",
    )
...
class Template(Base):
    ...
    is_public: Mapped[bool] = mapped_column(default=True, server_default="true", nullable=False)
...
class AppInvite(Base):
    ...
    __table_args__ = (
        CheckConstraint("role IN ('editor', 'viewer')", name="ck_app_invites_role"),
    )
